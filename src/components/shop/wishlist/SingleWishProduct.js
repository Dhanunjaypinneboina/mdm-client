...
const Product = () => {
  ...
  if (loading) {
    return (
      <div className="my-32 text-2xl text-center">Your wishList is empty</div>
    );
  }
  return (
    <Fragment>
        ...
        {products.length > 0 ? (
          products.map((product, index) => {
            return (
              <div
                key={index}
                className=" border border-dark p-2 relative m-2 md:py-6 md:border-t md:border-b md:my-2 md:mx-0 col-span-1 md:flex md:items-center md:justify-between"
              >
