...
const ShampooAndOils = (props) => {
  const { data, dispatch } = useContext(HomeContext);
  // const { premiumProducts } = data;
  const { shampoosAndOils } = data;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (responseData && responseData.Products) {
        shampoosAndOils = responseData.Products.filter(
          (product) =>
            product.pCategory.cName === "Hair Oils" ||
            product.pCategory.cName === "Shampoos"
        );
      }
      ...
      setTimeout(() => {
        if (shampoosAndOils) {
          dispatch({
            type: "setOilsAndShampoos",
            payload: shampoosAndOils,
          });
  ...
  return (
    <Fragment>
      {shampoosAndOils && shampoosAndOils.length > 0 ? (
        shampoosAndOils.map((item, index) => {
          return (
            <Fragment key={index}>
