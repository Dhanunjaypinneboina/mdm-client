...
import { relatedProductsByCategory } from "./FetchApi";
...
const RelatedProducts = () => {
  ...
  const fetchData = async () => {
    try {
      const responseData = await relatedProductsByCategory(id);
      setTimeout(() => {
        if (responseData && responseData.RelatedProducts) {
          dispatch({
            type: "relatedProducts",
            payload: responseData.RelatedProducts,
          });
        }
      }, 500);
    } catch (error) {
    ...
    <Fragment>
      {relatedProducts && relatedProducts.length > 0 ? (
        relatedProducts.map((item, index) => {
          return (
            <Fragment key={index}>
              <div className="border border-dark p-2 relative col-span-1 m-2">
                <img
                  onClick={() => {
